        ...
        try:
            if os.path.exists(SOUND_CONFIG_FILE):
                with open(SOUND_CONFIG_FILE, 'r') as f:
                    return json.load(f)
            return {"sound_detections": []}
        except Exception as e:
            logging.error(f"Erreur lors du chargement de la configuration des sons: {str(e)}")
            return {"sound_detections": []}
    
    def save_config(self, new_config):
        """Sauvegarde la configuration des sons"""
        with self.lock:
            try:
                with open(SOUND_CONFIG_FILE, 'w') as f:
                    json.dump(new_config, f, indent=4)
                self.config = new_config
                return True, "Configuration des sons sauvegardée avec succès"
            except Exception as e:
                return False, f"Erreur lors de la sauvegarde de la configuration: {str(e)}"
    
    def get_config(self):
        """Retourne la configuration actuelle"""
        return self.config

# Initialiser la configuration des sons
sound_config = SoundDetectionConfig()
...
def test_webhook():
    ...
    try:
        ...
        if not url:
            return jsonify({"success": False, "error": "URL non fournie"}), 400
        ...
        logging.error(f"Erreur lors du test du webhook: {str(e)}")
...
def get_sound_config():
    """Retourne la configuration des sons à détecter"""
    return jsonify(sound_config.get_config())

@app.route('/api/yamnet-labels', methods=['GET'])
def get_yamnet_labels():
    """Retourne la liste des labels YAMNet"""
    try:
        labels = []
        with open(os.path.join(BASE_DIR, 'yamnet_class_map.csv'), 'r') as f:
            ...
            for line in f:
                ...
                if len(parts) >= 3:
                    # Utiliser le display_name comme label
                    labels.append(parts[2].strip('"'))
        return jsonify({"labels": labels})
    except Exception as e:
        logging.error(f"Erreur lors de la récupération des labels YAMNet: {str(e)}")
        return jsonify({"error": str(e)}), 500

@app.route('/api/sound-config', methods=['POST'])
def update_sound_config():
    """Met à jour la configuration des sons à détecter"""
    try:
        new_config = request.json
        success, message = sound_config.save_config(new_config)
        if success:
...
if __name__ == '__main__':
    try:
        ...
        socketio.run(app, host='0.0.0.0', port=16045, debug=True, allow_unsafe_werkzeug=True)
